...
class AddStory: UICollectionViewCell {
    ...
    func settingView() {
       ...
       // viewBottomView.clipsToBounds = true
        ...
        let imageNameURL = URL(string: UserDefaults.standard.string(forKey: StringUserDefault.userImageName)!)
        if let imageUrl = imageNameURL {
            imgUserAvatar.sd_setImage(with: imageUrl)
        }
    }
